    ...
    <i-table class="Url-table" :columns="columns1" :data="articlesData" :loading="loading" @on-selection-change="handleSelectRow()" ref="table" stripe border @on-filter-change="handleFilter" @on-sort-change="handleSort">
        <template slot-scope="{ row }" slot="action">
            <div class="Url-actions">
                <i-button type="primary" size="small" style="margin-right: 5px" @click="initVideo(row._id)">编辑</i-button>
            </div>
        </template>
    </i-table>
    <br>
...
export default {
    ...
    methods: {
        ...
        initVideo: function (_id){
            this.$router.push('/Project/'+this.currentProject+'/VideoEdit/'+_id)
        },
        ...
        handleFilter: function (column) {
            ...
            } else if (chekced.key === 'video_imageUrl') {
